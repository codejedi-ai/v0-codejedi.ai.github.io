...
export const NOTION_API_KEY = 'secret_8zGtazmQsLQXewj4QHkqOcwFoATEXDzpswRe1nty8Ca';
export const notion = new Client({ auth: NOTION_API_KEY });

const PORTFOLIO_BACKEND_NOTION_PAGE = "42ba51e735c6425282891f116d7c2e3f";
// const CERTIFICATE_DATABASE_ID = "7ad088a9fa3e42618eb4d140952aaa3f";
import * as fs from 'fs/promises';

// export the children of Portfolio Backend Notion Page
export async function fetchPortfolioBackendPageChildren() : Promise<(NotionAPI.PartialBlockObjectResponse | NotionAPI.BlockObjectResponse)[]> {
  try {
    const response = await notion.blocks.children.list({
      block_id: PORTFOLIO_BACKEND_NOTION_PAGE,
    });
...
async function fetchDatabaseMap(): Promise<DatabaseMap> {
  const list_db = await fetchPortfolioBackendPageChildren();
  const databaseMap: DatabaseMap = {};

  for (const db of list_db) {
    if ('type' in db && 
        db.type === 'child_database' && 
        db.type in db && 
        'title' in db[db.type]) {
      const title = db[db.type]?.title || 'Untitled';
      databaseMap[title] = db.id;
    }
  }

  return databaseMap;
}
const mapNameID = await fetchDatabaseMap();
export const WORK_EXPERIANCE_DATABASE_ID = mapNameID['work-experience'];
export const IMAGES_DATABASE_ID = mapNameID['images'];
export const CERTS_DATABASE_ID = mapNameID['certs'];
const jsonOutput = JSON.stringify(mapNameID, null, 2);
...
const filePath = path.join(folder, 'notion-homepage.json');
await fs.writeFile(filePath, jsonOutput, 'utf-8');
